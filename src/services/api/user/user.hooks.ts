...
import { AuthCredentialSubject, buildAuthCredentialSubject, buildEmailCredentialSubject, buildFirstNameCredentialSubject, buildKYCCredentialSubject, EmailCredentialSubject, FirstNameCredentialSubject, issueCredentialsHelper, KYCCredentialSubject } from '../../../utils/credentials';
...
export const issueAuthAndKYCAndEmailCredentials: UserServiceHook = async (ctx) => {
  ...
  const fistNameCredentialSubject: FirstNameCredentialSubject = buildFirstNameCredentialSubject(did, result.firstName as string || 'Richard');

  const issuerDto: UnumDto<CredentialPb[]> = await issueCredentialsHelper(defaultIssuerEntity, did, [authCredentialSubject, emailCredentialSubject, fistNameCredentialSubject]);
